...
if ($objMenuPpal) {
    //t('El programa principal tiene el Id: '.$objMenuPpal->Id);
}
//----------------------------------
...
if ($objOpciAdmi) {
    //t('El programa admin tiene el Id: '.$objOpciAdmi->Id);
}
//---------------------------------------------------------------------------------
...
if (!is_null($objOpciAdmi)) {
    ...
    if (!in_array($objUsuario->LogiUsua,$arrUsuaAdmi)) {
        ...
        //t('Exluyendo la opcion: '.$objOpciAdmi->Id);
    }
}
//-------------------------------
...
$objClauWher   = QQ::Clause();
$objClauWher[] = QQ::Equal(QQN::NewOpcion()->SistemaId,'ret');
$objClauWher[] = QQ::Equal(QQN::NewOpcion()->Dependencia,$objMenuPpal->Id);
...
foreach ($arrOpciMenu as $objOpcion) {
    $strHtmlMenu .= $objOpcion->HtmlMenuBootstrap('ret');
}
$strHtmlMenu .= "</ul>\n";
?>