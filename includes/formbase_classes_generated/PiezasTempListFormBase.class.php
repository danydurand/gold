 ...
 * PiezasTempDataGrid control which has meta-methods to help with
 * easily creating/defining PiezasTemp columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 *
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both piezas_temp_list.php AND
 * piezas_temp_list.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class PiezasTempListFormBase extends QForm {
    ...
    // Local instance of the Meta DataGrid to list PiezasTemps
    /**
     * @var PiezasTempDataGrid dtgPiezasTemps
     */
    protected $dtgPiezasTemps;
    ...
    protected function Form_Create() {
        ...
        $this->dtgPiezasTemps = new PiezasTempDataGrid($this);

        // Style the DataGrid (if desired)
        $this->dtgPiezasTemps->CssClass = 'datagrid';
        $this->dtgPiezasTemps->AlternateRowStyle->CssClass = 'alternate';
        $this->dtgPiezasTemps->FontSize = 13;
        $this->dtgPiezasTemps->ShowFilter = false;

        // Add Pagination (if desired)
        $this->dtgPiezasTemps->Paginator = new QPaginator($this->dtgPiezasTemps);
        $this->dtgPiezasTemps->ItemsPerPage = __FORM_DRAFTS_FORM_LIST_ITEMS_PER_PAGE__;

        // Higlight the datagrid rows when mousing over them
        $this->dtgPiezasTemps->AddRowAction(new QMouseOverEvent(), new QCssClassAction('selectedStyle'));
        $this->dtgPiezasTemps->AddRowAction(new QMouseOutEvent(), new QCssClassAction());

        // Add a click handler for the rows.
        // We can use $_CONTROL->CurrentRowIndex to pass the row index to dtgPersonsRow_Click()
        // or $_ITEM->Id to pass the object's id, or any other data grid variable
        $this->dtgPiezasTemps->RowActionParameterHtml = '<?= $_ITEM->Id ?>';
        $this->dtgPiezasTemps->AddRowAction(new QClickEvent(), new QAjaxAction('dtgPiezasTempsRow_Click'));

        // Use the MetaDataGrid functionality to add Columns for this datagrid

        // Create the Other Columns (note that you can use strings for piezas_temp's properties, or you
        ...
        $this->dtgPiezasTemps->MetaAddColumn('CreatedBy');
        $this->dtgPiezasTemps->MetaAddColumn('UpdatedBy');
    ...
    protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'PiezasTemps';
    }
    ...
    protected function btnExpoExce_Create() {
        $this->btnExpoExce = new QDataGridExporterButton($this, $this->dtgPiezasTemps);
    ...
    protected function btnFiltAvan_Click() {
        $this->dtgPiezasTemps->ShowFilter = !$this->dtgPiezasTemps->ShowFilter;
    }

    public function dtgPiezasTempsRow_Click($strFormId, $strControlId, $strParameter) {
      $intId = intval($strParameter);
      QApplication::Redirect("piezas_temp_edit.php/$intId");
    }
