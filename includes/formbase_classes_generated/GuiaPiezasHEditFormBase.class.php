 ...
 * of the GuiaPiezasH class.  It uses the code-generated
 * GuiaPiezasHMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a GuiaPiezasH columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both guia_piezas_h_edit.php AND
 * guia_piezas_h_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class GuiaPiezasHEditFormBase extends QForm {
	// Local instance of the GuiaPiezasHMetaControl
	/**
	 * @var GuiaPiezasHMetaControlGen mctGuiaPiezasH
	 */
	protected $mctGuiaPiezasH;
	...
	// Controls for GuiaPiezasH's Data Fields
	protected $lblId;
	protected $lstGuia;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the GuiaPiezasHMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctGuiaPiezasH = GuiaPiezasHMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on GuiaPiezasH's data fields
		$this->lblId = $this->mctGuiaPiezasH->lblId_Create();
		$this->lstGuia = $this->mctGuiaPiezasH->lstGuia_Create();
		$this->txtIdPieza = $this->mctGuiaPiezasH->txtIdPieza_Create();
		$this->txtKilos = $this->mctGuiaPiezasH->txtKilos_Create();
		$this->txtLibras = $this->mctGuiaPiezasH->txtLibras_Create();
		$this->txtLargo = $this->mctGuiaPiezasH->txtLargo_Create();
		$this->txtAlto = $this->mctGuiaPiezasH->txtAlto_Create();
		$this->txtAncho = $this->mctGuiaPiezasH->txtAncho_Create();
		$this->txtVolumen = $this->mctGuiaPiezasH->txtVolumen_Create();
		$this->txtDescripcion = $this->mctGuiaPiezasH->txtDescripcion_Create();
		$this->txtPiesCub = $this->mctGuiaPiezasH->txtPiesCub_Create();
		$this->txtMetrosCub = $this->mctGuiaPiezasH->txtMetrosCub_Create();
		$this->txtHojaEntrega = $this->mctGuiaPiezasH->txtHojaEntrega_Create();
		$this->txtUbicacion = $this->mctGuiaPiezasH->txtUbicacion_Create();
		$this->lblCreatedAt = $this->mctGuiaPiezasH->lblCreatedAt_Create();
		$this->lblUpdatedAt = $this->mctGuiaPiezasH->lblUpdatedAt_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctGuiaPiezasH->GuiaPiezasH && !isset($_SESSION['DataGuiaPiezasH'])) {
            $_SESSION['DataGuiaPiezasH'] = serialize(array($this->mctGuiaPiezasH->GuiaPiezasH));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataGuiaPiezasH']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctGuiaPiezasH->GuiaPiezasH->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'GuiaPiezasH';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctGuiaPiezasH->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('GuiaPiezasH'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctGuiaPiezasH->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('GuiaPiezasH',$this->mctGuiaPiezasH->GuiaPiezasH->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctGuiaPiezasH->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctGuiaPiezasH->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objGuiaPiezasH = GuiaPiezasH::LoadByIdPieza($this->txtIdPieza->Text)) && ($objGuiaPiezasH->Id != $this->mctGuiaPiezasH->GuiaPiezasH->Id )){
				$blnToReturn = false;
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/guia_piezas_h_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctGuiaPiezasH->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the GuiaPiezasHMetaControl
		$this->mctGuiaPiezasH->SaveGuiaPiezasH();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctGuiaPiezasH->TablasRelacionadasGuiaPiezasH();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the GuiaPiezasHMetaControl
			$this->mctGuiaPiezasH->DeleteGuiaPiezasH();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctGuiaPiezasH->GuiaPiezasH->Id;
        $_SESSION['TablRefe'] = 'GuiaPiezasH';
        $_SESSION['RegiReto'] = 'guia_piezas_h_edit.php/'.$this->mctGuiaPiezasH->GuiaPiezasH->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/guia_piezas_h_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/guia_piezas_h_edit.php');
    }
