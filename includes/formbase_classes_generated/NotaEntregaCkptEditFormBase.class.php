 ...
 * NotaEntregaCkptMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a NotaEntregaCkpt columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both nota_entrega_ckpt_edit.php AND
 * nota_entrega_ckpt_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class NotaEntregaCkptEditFormBase extends QForm {
	// Local instance of the NotaEntregaCkptMetaControl
	/**
	 * @var NotaEntregaCkptMetaControlGen mctNotaEntregaCkpt
	 */
	protected $mctNotaEntregaCkpt;
	...
	// Controls for NotaEntregaCkpt's Data Fields
	protected $lblId;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the NotaEntregaCkptMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctNotaEntregaCkpt = NotaEntregaCkptMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on NotaEntregaCkpt's data fields
		$this->lblId = $this->mctNotaEntregaCkpt->lblId_Create();
		$this->lstContainer = $this->mctNotaEntregaCkpt->lstContainer_Create();
		$this->lstCheckpoint = $this->mctNotaEntregaCkpt->lstCheckpoint_Create();
		$this->lstSucursal = $this->mctNotaEntregaCkpt->lstSucursal_Create();
		$this->calFecha = $this->mctNotaEntregaCkpt->calFecha_Create();
		$this->txtHora = $this->mctNotaEntregaCkpt->txtHora_Create();
		$this->lstUsuario = $this->mctNotaEntregaCkpt->lstUsuario_Create();
		$this->txtObservacion = $this->mctNotaEntregaCkpt->txtObservacion_Create();
		$this->calCreatedAt = $this->mctNotaEntregaCkpt->calCreatedAt_Create();
		$this->calUpdatedAt = $this->mctNotaEntregaCkpt->calUpdatedAt_Create();
		$this->txtCreatedBy = $this->mctNotaEntregaCkpt->txtCreatedBy_Create();
		$this->txtUpdatedBy = $this->mctNotaEntregaCkpt->txtUpdatedBy_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctNotaEntregaCkpt->NotaEntregaCkpt && !isset($_SESSION['DataNotaEntregaCkpt'])) {
            $_SESSION['DataNotaEntregaCkpt'] = serialize(array($this->mctNotaEntregaCkpt->NotaEntregaCkpt));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataNotaEntregaCkpt']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctNotaEntregaCkpt->NotaEntregaCkpt->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'NotaEntregaCkpt';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctNotaEntregaCkpt->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('NotaEntregaCkpt'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctNotaEntregaCkpt->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('NotaEntregaCkpt',$this->mctNotaEntregaCkpt->NotaEntregaCkpt->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctNotaEntregaCkpt->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctNotaEntregaCkpt->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/nota_entrega_ckpt_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctNotaEntregaCkpt->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the NotaEntregaCkptMetaControl
		$this->mctNotaEntregaCkpt->SaveNotaEntregaCkpt();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctNotaEntregaCkpt->TablasRelacionadasNotaEntregaCkpt();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the NotaEntregaCkptMetaControl
			$this->mctNotaEntregaCkpt->DeleteNotaEntregaCkpt();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctNotaEntregaCkpt->NotaEntregaCkpt->Id;
        $_SESSION['TablRefe'] = 'NotaEntregaCkpt';
        $_SESSION['RegiReto'] = 'nota_entrega_ckpt_edit.php/'.$this->mctNotaEntregaCkpt->NotaEntregaCkpt->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/nota_entrega_ckpt_list.php');
    }
