 ...
 * of the TempSubcuentas class.  It uses the code-generated
 * TempSubcuentasMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a TempSubcuentas columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both temp_subcuentas_edit.php AND
 * temp_subcuentas_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class TempSubcuentasEditFormBase extends QForm {
	// Local instance of the TempSubcuentasMetaControl
	/**
	 * @var TempSubcuentasMetaControlGen mctTempSubcuentas
	 */
	protected $mctTempSubcuentas;
	...
	// Controls for TempSubcuentas's Data Fields
	protected $txtId;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the TempSubcuentasMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctTempSubcuentas = TempSubcuentasMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on TempSubcuentas's data fields
		$this->txtId = $this->mctTempSubcuentas->txtId_Create();
		$this->txtCodigoInterno = $this->mctTempSubcuentas->txtCodigoInterno_Create();
		$this->txtEstatus = $this->mctTempSubcuentas->txtEstatus_Create();
		$this->txtUsuarioApi = $this->mctTempSubcuentas->txtUsuarioApi_Create();
		$this->txtPasswordApi = $this->mctTempSubcuentas->txtPasswordApi_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctTempSubcuentas->TempSubcuentas && !isset($_SESSION['DataTempSubcuentas'])) {
            $_SESSION['DataTempSubcuentas'] = serialize(array($this->mctTempSubcuentas->TempSubcuentas));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataTempSubcuentas']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctTempSubcuentas->TempSubcuentas->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'TempSubcuentas';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctTempSubcuentas->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('TempSubcuentas'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctTempSubcuentas->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('TempSubcuentas',$this->mctTempSubcuentas->TempSubcuentas->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctTempSubcuentas->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctTempSubcuentas->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objTempSubcuentas = TempSubcuentas::LoadByCodigoInterno($this->txtCodigoInterno->Text)) && ($objTempSubcuentas->Id != $this->mctTempSubcuentas->TempSubcuentas->Id )){
				$blnToReturn = false;
				$this->txtCodigoInterno->Warning = QApplication::Translate("Already in Use");
			}
			if (($objTempSubcuentas = TempSubcuentas::LoadByUsuarioApi($this->txtUsuarioApi->Text)) && ($objTempSubcuentas->Id != $this->mctTempSubcuentas->TempSubcuentas->Id )){
				$blnToReturn = false;
				$this->txtUsuarioApi->Warning = QApplication::Translate("Already in Use");
			}
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/temp_subcuentas_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctTempSubcuentas->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the TempSubcuentasMetaControl
		$this->mctTempSubcuentas->SaveTempSubcuentas();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctTempSubcuentas->TablasRelacionadasTempSubcuentas();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the TempSubcuentasMetaControl
			$this->mctTempSubcuentas->DeleteTempSubcuentas();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctTempSubcuentas->TempSubcuentas->Id;
        $_SESSION['TablRefe'] = 'TempSubcuentas';
        $_SESSION['RegiReto'] = 'temp_subcuentas_edit.php/'.$this->mctTempSubcuentas->TempSubcuentas->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/temp_subcuentas_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/temp_subcuentas_edit.php');
    }
