 ...
 * of the ClientesInternacional class.  It uses the code-generated
 * ClientesInternacionalMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a ClientesInternacional columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both clientes_internacional_edit.php AND
 * clientes_internacional_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class ClientesInternacionalEditFormBase extends QForm {
	// Local instance of the ClientesInternacionalMetaControl
	/**
	 * @var ClientesInternacionalMetaControlGen mctClientesInternacional
	 */
	protected $mctClientesInternacional;
	...
	// Controls for ClientesInternacional's Data Fields
	protected $lblId;
	protected $txtCodigo;
	protected $txtNombre;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the ClientesInternacionalMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctClientesInternacional = ClientesInternacionalMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on ClientesInternacional's data fields
		$this->lblId = $this->mctClientesInternacional->lblId_Create();
		$this->txtCodigo = $this->mctClientesInternacional->txtCodigo_Create();
		$this->txtNombre = $this->mctClientesInternacional->txtNombre_Create();
		$this->lstSucursal = $this->mctClientesInternacional->lstSucursal_Create();
		$this->txtTelefonoMovil = $this->mctClientesInternacional->txtTelefonoMovil_Create();
		$this->txtCedulaRif = $this->mctClientesInternacional->txtCedulaRif_Create();
		$this->txtSexo = $this->mctClientesInternacional->txtSexo_Create();
		$this->txtEmail = $this->mctClientesInternacional->txtEmail_Create();
		$this->txtTelefonoFijo = $this->mctClientesInternacional->txtTelefonoFijo_Create();
		$this->txtDireccion = $this->mctClientesInternacional->txtDireccion_Create();
		$this->calFechaNacimiento = $this->mctClientesInternacional->calFechaNacimiento_Create();
		$this->lstProfesion = $this->mctClientesInternacional->lstProfesion_Create();
		$this->lblCreatedAt = $this->mctClientesInternacional->lblCreatedAt_Create();
		$this->lblUpdatedAt = $this->mctClientesInternacional->lblUpdatedAt_Create();
		$this->lblDeletedAt = $this->mctClientesInternacional->lblDeletedAt_Create();
		$this->txtCreatedBy = $this->mctClientesInternacional->txtCreatedBy_Create();
		$this->txtUpdatedBy = $this->mctClientesInternacional->txtUpdatedBy_Create();
		$this->txtDeletedBy = $this->mctClientesInternacional->txtDeletedBy_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctClientesInternacional->ClientesInternacional && !isset($_SESSION['DataClientesInternacional'])) {
            $_SESSION['DataClientesInternacional'] = serialize(array($this->mctClientesInternacional->ClientesInternacional));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataClientesInternacional']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctClientesInternacional->ClientesInternacional->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'ClientesInternacional';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctClientesInternacional->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('ClientesInternacional'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctClientesInternacional->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('ClientesInternacional',$this->mctClientesInternacional->ClientesInternacional->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctClientesInternacional->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctClientesInternacional->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/clientes_internacional_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctClientesInternacional->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the ClientesInternacionalMetaControl
		$this->mctClientesInternacional->SaveClientesInternacional();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctClientesInternacional->TablasRelacionadasClientesInternacional();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the ClientesInternacionalMetaControl
			$this->mctClientesInternacional->DeleteClientesInternacional();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctClientesInternacional->ClientesInternacional->Id;
        $_SESSION['TablRefe'] = 'ClientesInternacional';
        $_SESSION['RegiReto'] = 'clientes_internacional_edit.php/'.$this->mctClientesInternacional->ClientesInternacional->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/clientes_internacional_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/clientes_internacional_edit.php');
    }
