 ...
 * of the NotaEntregaZona class.  It uses the code-generated
 * NotaEntregaZonaMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a NotaEntregaZona columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both nota_entrega_zona_edit.php AND
 * nota_entrega_zona_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class NotaEntregaZonaEditFormBase extends QForm {
	// Local instance of the NotaEntregaZonaMetaControl
	/**
	 * @var NotaEntregaZonaMetaControlGen mctNotaEntregaZona
	 */
	protected $mctNotaEntregaZona;
	...
	// Controls for NotaEntregaZona's Data Fields
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the NotaEntregaZonaMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctNotaEntregaZona = NotaEntregaZonaMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on NotaEntregaZona's data fields
		$this->lstNotaEntrega = $this->mctNotaEntregaZona->lstNotaEntrega_Create();
		$this->txtZona = $this->mctNotaEntregaZona->txtZona_Create();
		$this->txtPiezas = $this->mctNotaEntregaZona->txtPiezas_Create();
		$this->txtKilos = $this->mctNotaEntregaZona->txtKilos_Create();
		$this->txtPiesCub = $this->mctNotaEntregaZona->txtPiesCub_Create();
		$this->txtPrecio = $this->mctNotaEntregaZona->txtPrecio_Create();
		$this->txtTotal = $this->mctNotaEntregaZona->txtTotal_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctNotaEntregaZona->NotaEntregaZona && !isset($_SESSION['DataNotaEntregaZona'])) {
            $_SESSION['DataNotaEntregaZona'] = serialize(array($this->mctNotaEntregaZona->NotaEntregaZona));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataNotaEntregaZona']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctNotaEntregaZona->NotaEntregaZona->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'NotaEntregaZona';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctNotaEntregaZona->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('NotaEntregaZona'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctNotaEntregaZona->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('NotaEntregaZona',$this->mctNotaEntregaZona->NotaEntregaZona->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctNotaEntregaZona->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctNotaEntregaZona->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/nota_entrega_zona_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctNotaEntregaZona->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the NotaEntregaZonaMetaControl
		$this->mctNotaEntregaZona->SaveNotaEntregaZona();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctNotaEntregaZona->TablasRelacionadasNotaEntregaZona();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->lstNotaEntrega->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->txtZona->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the NotaEntregaZonaMetaControl
			$this->mctNotaEntregaZona->DeleteNotaEntregaZona();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctNotaEntregaZona->NotaEntregaZona->Id;
        $_SESSION['TablRefe'] = 'NotaEntregaZona';
        $_SESSION['RegiReto'] = 'nota_entrega_zona_edit.php/'.$this->mctNotaEntregaZona->NotaEntregaZona->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/nota_entrega_zona_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/nota_entrega_zona_edit.php');
    }
