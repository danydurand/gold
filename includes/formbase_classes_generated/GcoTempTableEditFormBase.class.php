 ...
 * of the GcoTempTable class.  It uses the code-generated
 * GcoTempTableMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a GcoTempTable columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both gco_temp_table_edit.php AND
 * gco_temp_table_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class GcoTempTableEditFormBase extends QForm {
	// Local instance of the GcoTempTableMetaControl
	/**
	 * @var GcoTempTableMetaControlGen mctGcoTempTable
	 */
	protected $mctGcoTempTable;
	...
	// Controls for GcoTempTable's Data Fields
	protected $lblId;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the GcoTempTableMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctGcoTempTable = GcoTempTableMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on GcoTempTable's data fields
		$this->lblId = $this->mctGcoTempTable->lblId_Create();
		$this->lstProcesoError = $this->mctGcoTempTable->lstProcesoError_Create();
		$this->lstGuia = $this->mctGcoTempTable->lstGuia_Create();
		$this->lstConcepto = $this->mctGcoTempTable->lstConcepto_Create();
		$this->calCreatedAt = $this->mctGcoTempTable->calCreatedAt_Create();
		$this->calUpdatedAt = $this->mctGcoTempTable->calUpdatedAt_Create();
		$this->lstCreatedByObject = $this->mctGcoTempTable->lstCreatedByObject_Create();
		$this->lstUpdatedByObject = $this->mctGcoTempTable->lstUpdatedByObject_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctGcoTempTable->GcoTempTable && !isset($_SESSION['DataGcoTempTable'])) {
            $_SESSION['DataGcoTempTable'] = serialize(array($this->mctGcoTempTable->GcoTempTable));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataGcoTempTable']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctGcoTempTable->GcoTempTable->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'GcoTempTable';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctGcoTempTable->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('GcoTempTable'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctGcoTempTable->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('GcoTempTable',$this->mctGcoTempTable->GcoTempTable->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctGcoTempTable->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctGcoTempTable->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objGcoTempTable = GcoTempTable::LoadByGuiaIdConceptoId($this->lstGuia->SelectedValue,$this->lstConcepto->SelectedValue)) && ($objGcoTempTable->Id != $this->mctGcoTempTable->GcoTempTable->Id )){
				$blnToReturn = false;
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/gco_temp_table_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctGcoTempTable->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the GcoTempTableMetaControl
		$this->mctGcoTempTable->SaveGcoTempTable();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctGcoTempTable->TablasRelacionadasGcoTempTable();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the GcoTempTableMetaControl
			$this->mctGcoTempTable->DeleteGcoTempTable();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctGcoTempTable->GcoTempTable->Id;
        $_SESSION['TablRefe'] = 'GcoTempTable';
        $_SESSION['RegiReto'] = 'gco_temp_table_edit.php/'.$this->mctGcoTempTable->GcoTempTable->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/gco_temp_table_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/gco_temp_table_edit.php');
    }
