 ...
 * GuiasHMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a GuiasH columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both guias_h_edit.php AND
 * guias_h_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class GuiasHEditFormBase extends QForm {
	// Local instance of the GuiasHMetaControl
	/**
	 * @var GuiasHMetaControlGen mctGuiasH
	 */
	protected $mctGuiasH;
	...
	// Controls for GuiasH's Data Fields
	protected $lblId;
	protected $txtNumero;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the GuiasHMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctGuiasH = GuiasHMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on GuiasH's data fields
		$this->lblId = $this->mctGuiasH->lblId_Create();
		$this->txtNumero = $this->mctGuiasH->txtNumero_Create();
		$this->txtTracking = $this->mctGuiasH->txtTracking_Create();
		$this->calFecha = $this->mctGuiasH->calFecha_Create();
		$this->lstClienteRetail = $this->mctGuiasH->lstClienteRetail_Create();
		$this->lstClienteCorp = $this->mctGuiasH->lstClienteCorp_Create();
		$this->lstClienteInt = $this->mctGuiasH->lstClienteInt_Create();
		$this->lstOrigen = $this->mctGuiasH->lstOrigen_Create();
		$this->lstDestino = $this->mctGuiasH->lstDestino_Create();
		$this->txtServicioEntrega = $this->mctGuiasH->txtServicioEntrega_Create();
		$this->txtServicioImportacion = $this->mctGuiasH->txtServicioImportacion_Create();
		$this->lstProducto = $this->mctGuiasH->lstProducto_Create();
		$this->txtFormaPago = $this->mctGuiasH->txtFormaPago_Create();
		$this->txtNombreRemitente = $this->mctGuiasH->txtNombreRemitente_Create();
		$this->txtDireccionRemitente = $this->mctGuiasH->txtDireccionRemitente_Create();
		$this->txtTelefonoRemitente = $this->mctGuiasH->txtTelefonoRemitente_Create();
		$this->txtNombreDestinatario = $this->mctGuiasH->txtNombreDestinatario_Create();
		$this->txtDireccionDestinatario = $this->mctGuiasH->txtDireccionDestinatario_Create();
		$this->txtTelefonoDestinatario = $this->mctGuiasH->txtTelefonoDestinatario_Create();
		$this->txtContenido = $this->mctGuiasH->txtContenido_Create();
		$this->txtPiezas = $this->mctGuiasH->txtPiezas_Create();
		$this->txtValorDeclarado = $this->mctGuiasH->txtValorDeclarado_Create();
		$this->txtTipoExport = $this->mctGuiasH->txtTipoExport_Create();
		$this->chkAsegurado = $this->mctGuiasH->chkAsegurado_Create();
		$this->txtTotal = $this->mctGuiasH->txtTotal_Create();
		$this->txtEstado = $this->mctGuiasH->txtEstado_Create();
		$this->txtCiudad = $this->mctGuiasH->txtCiudad_Create();
		$this->txtCodigoPostal = $this->mctGuiasH->txtCodigoPostal_Create();
		$this->txtTasa = $this->mctGuiasH->txtTasa_Create();
		$this->txtUbicacion = $this->mctGuiasH->txtUbicacion_Create();
		$this->lstVendedor = $this->mctGuiasH->lstVendedor_Create();
		$this->lstTarifa = $this->mctGuiasH->lstTarifa_Create();
		$this->lstTarifaAgente = $this->mctGuiasH->lstTarifaAgente_Create();
		$this->lstReceptoriaOrigen = $this->mctGuiasH->lstReceptoriaOrigen_Create();
		$this->lstReceptoriaDestino = $this->mctGuiasH->lstReceptoriaDestino_Create();
		$this->txtKilos = $this->mctGuiasH->txtKilos_Create();
		$this->txtLibras = $this->mctGuiasH->txtLibras_Create();
		$this->txtLargo = $this->mctGuiasH->txtLargo_Create();
		$this->txtAncho = $this->mctGuiasH->txtAncho_Create();
		$this->txtAlto = $this->mctGuiasH->txtAlto_Create();
		$this->txtVolumen = $this->mctGuiasH->txtVolumen_Create();
		$this->txtPiesCub = $this->mctGuiasH->txtPiesCub_Create();
		$this->txtMetrosCub = $this->mctGuiasH->txtMetrosCub_Create();
		$this->txtCedulaDestinatario = $this->mctGuiasH->txtCedulaDestinatario_Create();
		$this->txtFacturaId = $this->mctGuiasH->txtFacturaId_Create();
		$this->lstGuiaPod = $this->mctGuiasH->lstGuiaPod_Create();
		$this->lstNotaEntrega = $this->mctGuiasH->lstNotaEntrega_Create();
		$this->txtObservacion = $this->mctGuiasH->txtObservacion_Create();
		$this->lblCreatedAt = $this->mctGuiasH->lblCreatedAt_Create();
		$this->lblUpdatedAt = $this->mctGuiasH->lblUpdatedAt_Create();
		$this->lblDeletedAt = $this->mctGuiasH->lblDeletedAt_Create();
		$this->txtCreatedBy = $this->mctGuiasH->txtCreatedBy_Create();
		$this->txtUpdatedBy = $this->mctGuiasH->txtUpdatedBy_Create();
		$this->txtDeletedBy = $this->mctGuiasH->txtDeletedBy_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctGuiasH->GuiasH && !isset($_SESSION['DataGuiasH'])) {
            $_SESSION['DataGuiasH'] = serialize(array($this->mctGuiasH->GuiasH));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataGuiasH']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctGuiasH->GuiasH->Id) {
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctGuiasH->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('GuiasH'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctGuiasH->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('GuiasH',$this->mctGuiasH->GuiasH->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctGuiasH->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctGuiasH->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objGuiasH = GuiasH::LoadByNumero($this->txtNumero->Text)) && ($objGuiasH->Id != $this->mctGuiasH->GuiasH->Id )){
				$blnToReturn = false;
				$this->txtNumero->Warning = QApplication::Translate("Already in Use");
			}
			if (($objGuiasH = GuiasH::LoadByTracking($this->txtTracking->Text)) && ($objGuiasH->Id != $this->mctGuiasH->GuiasH->Id )){
				$blnToReturn = false;
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/guias_h_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the GuiasHMetaControl
		$this->mctGuiasH->SaveGuiasH();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctGuiasH->TablasRelacionadasGuiasH();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the GuiasHMetaControl
			$this->mctGuiasH->DeleteGuiasH();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctGuiasH->GuiasH->Id;
        $_SESSION['TablRefe'] = 'GuiasH';
        $_SESSION['RegiReto'] = 'guias_h_edit.php/'.$this->mctGuiasH->GuiasH->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/guias_h_list.php');
    }
