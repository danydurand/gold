 ...
 * of the PiezaCheckpointsH class.  It uses the code-generated
 * PiezaCheckpointsHMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a PiezaCheckpointsH columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both pieza_checkpoints_h_edit.php AND
 * pieza_checkpoints_h_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class PiezaCheckpointsHEditFormBase extends QForm {
	// Local instance of the PiezaCheckpointsHMetaControl
	/**
	 * @var PiezaCheckpointsHMetaControlGen mctPiezaCheckpointsH
	 */
	protected $mctPiezaCheckpointsH;
	...
	// Controls for PiezaCheckpointsH's Data Fields
	protected $lblId;
	protected $lstPieza;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the PiezaCheckpointsHMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctPiezaCheckpointsH = PiezaCheckpointsHMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on PiezaCheckpointsH's data fields
		$this->lblId = $this->mctPiezaCheckpointsH->lblId_Create();
		$this->lstPieza = $this->mctPiezaCheckpointsH->lstPieza_Create();
		$this->lstCheckpoint = $this->mctPiezaCheckpointsH->lstCheckpoint_Create();
		$this->lstSucursal = $this->mctPiezaCheckpointsH->lstSucursal_Create();
		$this->calFecha = $this->mctPiezaCheckpointsH->calFecha_Create();
		$this->txtHora = $this->mctPiezaCheckpointsH->txtHora_Create();
		$this->txtComentario = $this->mctPiezaCheckpointsH->txtComentario_Create();
		$this->lstRuta = $this->mctPiezaCheckpointsH->lstRuta_Create();
		$this->lblCreatedAt = $this->mctPiezaCheckpointsH->lblCreatedAt_Create();
		$this->lblUpdatedAt = $this->mctPiezaCheckpointsH->lblUpdatedAt_Create();
		$this->txtCreatedBy = $this->mctPiezaCheckpointsH->txtCreatedBy_Create();
		$this->txtUpdatedBy = $this->mctPiezaCheckpointsH->txtUpdatedBy_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctPiezaCheckpointsH->PiezaCheckpointsH && !isset($_SESSION['DataPiezaCheckpointsH'])) {
            $_SESSION['DataPiezaCheckpointsH'] = serialize(array($this->mctPiezaCheckpointsH->PiezaCheckpointsH));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataPiezaCheckpointsH']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctPiezaCheckpointsH->PiezaCheckpointsH->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'PiezaCheckpointsH';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctPiezaCheckpointsH->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('PiezaCheckpointsH'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctPiezaCheckpointsH->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('PiezaCheckpointsH',$this->mctPiezaCheckpointsH->PiezaCheckpointsH->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctPiezaCheckpointsH->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctPiezaCheckpointsH->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/pieza_checkpoints_h_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctPiezaCheckpointsH->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the PiezaCheckpointsHMetaControl
		$this->mctPiezaCheckpointsH->SavePiezaCheckpointsH();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctPiezaCheckpointsH->TablasRelacionadasPiezaCheckpointsH();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the PiezaCheckpointsHMetaControl
			$this->mctPiezaCheckpointsH->DeletePiezaCheckpointsH();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctPiezaCheckpointsH->PiezaCheckpointsH->Id;
        $_SESSION['TablRefe'] = 'PiezaCheckpointsH';
        $_SESSION['RegiReto'] = 'pieza_checkpoints_h_edit.php/'.$this->mctPiezaCheckpointsH->PiezaCheckpointsH->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/pieza_checkpoints_h_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/pieza_checkpoints_h_edit.php');
    }
