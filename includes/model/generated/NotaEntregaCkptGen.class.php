<?php
	/**
	 * The abstract NotaEntregaCkptGen class defined here is
	 * code-generated and contains all the basic CRUD-type functionality as well as
	 * basic methods to handle relationships and index-based loading.
	 *
	 * To use, you should use the NotaEntregaCkpt subclass which
	 * extends this NotaEntregaCkptGen class.
	 *
	 * Because subsequent re-code generations will overwrite any changes to this
	 * file, you should leave this file unaltered to prevent yourself from losing
	 * any information or code changes.  All customizations should be done by
	 * overriding existing or implementing new methods, properties and variables
	 * in the NotaEntregaCkpt class.
	 *
	 ...
	 * @property NotaEntrega $Container the value for the NotaEntrega object referenced by intContainerId (Not Null)
	 ...
	 */
	class NotaEntregaCkptGen extends QBaseClass implements IteratorAggregate {
		...
		/**
		 * Protected member variable that maps to the database PK Identity column nota_entrega_ckpt.id
		 * @var integer intId
		 */
		protected $intId;
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.container_id
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.checkpoint_id
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.sucursal_id
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.fecha
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.hora
		 * @var string strHora
		 */
		protected $strHora;
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.usuario_id
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.observacion
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.created_at
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.updated_at
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.created_by
		...
		/**
		 * Protected member variable that maps to the database column nota_entrega_ckpt.updated_by
		 ...
		 * in the database column nota_entrega_ckpt.container_id.
		 *
		 * NOTE: Always use the Container property getter to correctly retrieve this NotaEntrega object.
		 * (Because this class implements late binding, this variable reference MAY be null.)
		 * @var NotaEntrega objContainer
		 */
		protected $objContainer;

		/**
		 * Protected member variable that contains the object pointed by the reference
		 * in the database column nota_entrega_ckpt.checkpoint_id.
		 *
		...
		/**
		 * Protected member variable that contains the object pointed by the reference
		 * in the database column nota_entrega_ckpt.sucursal_id.
		 *
		...
		/**
		 * Protected member variable that contains the object pointed by the reference
		 * in the database column nota_entrega_ckpt.usuario_id.
		 *
		...
		{
			$this->intId = NotaEntregaCkpt::IdDefault;
			$this->intContainerId = NotaEntregaCkpt::ContainerIdDefault;
			$this->intCheckpointId = NotaEntregaCkpt::CheckpointIdDefault;
			$this->intSucursalId = NotaEntregaCkpt::SucursalIdDefault;
			$this->dttFecha = (NotaEntregaCkpt::FechaDefault === null)?null:new QDateTime(NotaEntregaCkpt::FechaDefault);
			$this->strHora = NotaEntregaCkpt::HoraDefault;
			$this->intUsuarioId = NotaEntregaCkpt::UsuarioIdDefault;
			$this->strObservacion = NotaEntregaCkpt::ObservacionDefault;
			$this->dttCreatedAt = (NotaEntregaCkpt::CreatedAtDefault === null)?null:new QDateTime(NotaEntregaCkpt::CreatedAtDefault);
			$this->dttUpdatedAt = (NotaEntregaCkpt::UpdatedAtDefault === null)?null:new QDateTime(NotaEntregaCkpt::UpdatedAtDefault);
			$this->intCreatedBy = NotaEntregaCkpt::CreatedByDefault;
			$this->intUpdatedBy = NotaEntregaCkpt::UpdatedByDefault;
		}
		...
		/**
		 * Load a NotaEntregaCkpt from PK Info
		 * @param integer $intId
		...
		public static function Load($intId, $objOptionalClauses = null) {
			...
			if (QApplication::$objCacheProvider && !$objOptionalClauses && QApplication::$Database[1]->Caching) {
				$strCacheKey = QApplication::$objCacheProvider->CreateKey(QApplication::$Database[1]->Database, 'NotaEntregaCkpt', $intId);
			...
			$objToReturn = NotaEntregaCkpt::QuerySingle(
				QQ::AndCondition(
					QQ::Equal(QQN::NotaEntregaCkpt()->Id, $intId)
				),
				$objOptionalClauses
			);
		...
		public static function LoadAll($objOptionalClauses = null) {
			...
			// Call NotaEntregaCkpt::QueryArray to perform the LoadAll query
			try {
				return NotaEntregaCkpt::QueryArray(QQ::All(), $objOptionalClauses);
		...
		/**
		 * Count all NotaEntregaCkpts
		 * @return int
		 */
		public static function CountAll() {
			// Call NotaEntregaCkpt::QueryCount to perform the CountAll query
			return NotaEntregaCkpt::QueryCount(QQ::All());
		}
		...
		protected static function BuildQueryStatement(&$objQueryBuilder, QQCondition $objConditions, $objOptionalClauses, $mixParameterArray, $blnCountOnly) {
			...
			$objDatabase = NotaEntregaCkpt::GetDatabase();

			// Create/Build out the QueryBuilder object with NotaEntregaCkpt-specific SELET and FROM fields
			$objQueryBuilder = new QQueryBuilder($objDatabase, 'nota_entrega_ckpt');
			...
			if ($blnAddAllFieldsToSelect) {
				NotaEntregaCkpt::GetSelectFields($objQueryBuilder, null, QQuery::extractSelectClause($objOptionalClauses));
			}
			$objQueryBuilder->AddFromItem('nota_entrega_ckpt');
		...
		/**
		 * Static Qcubed Query method to query for a single NotaEntregaCkpt object.
		 * Uses BuildQueryStatment to perform most of the work.
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		 * @return NotaEntregaCkpt the queried object
		 */
		public static function QuerySingle(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			// Get the Query Statement
			try {
				$strQuery = NotaEntregaCkpt::BuildQueryStatement($objQueryBuilder, $objConditions, $objOptionalClauses, $mixParameterArray, false);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}

			// Perform the Query, Get the First Row, and Instantiate a new NotaEntregaCkpt object
			...
			if ($objQueryBuilder->ExpandAsArrayNode) {
				...
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = NotaEntregaCkpt::InstantiateDbRow($objDbRow, null, $objQueryBuilder->ExpandAsArrayNode, $objPrevItemArray, $objQueryBuilder->ColumnAliasArray);
					if ($objItem) {
						$objToReturn[] = $objItem;
				...
				if(null === $objDbRow)
					return null;
				return NotaEntregaCkpt::InstantiateDbRow($objDbRow, null, null, null, $objQueryBuilder->ColumnAliasArray);
			}
		}

		/**
		 * Static Qcubed Query method to query for an array of NotaEntregaCkpt objects.
		 * Uses BuildQueryStatment to perform most of the work.
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		 * @return NotaEntregaCkpt[] the queried objects as an array
		 */
		public static function QueryArray(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			...
			$objDbResult = $objQueryBuilder->Database->Query($strQuery);
			return NotaEntregaCkpt::InstantiateDbResult($objDbResult, $objQueryBuilder->ExpandAsArrayNode, $objQueryBuilder->ColumnAliasArray);
		}
		...
		/**
		 * Static Qcubed Query method to query for a count of NotaEntregaCkpt objects.
		 * Uses BuildQueryStatment to perform most of the work.
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		...
		public static function QueryCount(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			// Get the Query Statement
			try {
				$strQuery = NotaEntregaCkpt::BuildQueryStatement($objQueryBuilder, $objConditions, $objOptionalClauses, $mixParameterArray, true);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
		...
		public static function QueryArrayCached(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null, $blnForceUpdate = false) {
			...
			$objCache = new QCache('qquery/notaentregackpt', $strQuery);
			$cacheData = $objCache->GetData();

			if (!$cacheData || $blnForceUpdate) {
				$objDbResult = $objQueryBuilder->Database->Query($strQuery);
				$arrResult = NotaEntregaCkpt::InstantiateDbResult($objDbResult, $objQueryBuilder->ExpandAsArrayNode, $objQueryBuilder->ColumnAliasArray);
		...
		/**
		 * Updates a QQueryBuilder with the SELECT fields for this NotaEntregaCkpt
		...
		public static function GetSelectFields(QQueryBuilder $objBuilder, $strPrefix = null, QQSelect $objSelect = null) {
			if ($strPrefix) {
				...
				$strTableName = 'nota_entrega_ckpt';
				$strAliasPrefix = '';
			}
		...
		/**
		 * Instantiate a NotaEntregaCkpt from a Database Row.
		 * Takes in an optional strAliasPrefix, used in case another Object::InstantiateDbRow
		 * is calling this NotaEntregaCkpt::InstantiateDbRow in order to perform
		 ...
		 * @param string[] $strColumnAliasArray
		 * @return mixed Either a NotaEntregaCkpt, or false to indicate the dbrow was used in an expansion, or null to indicate that this leaf is a duplicate.
		*/
		public static function InstantiateDbRow($objDbRow, $strAliasPrefix = null, $objExpandAsArrayNode = null, $objPreviousItemArray = null, $strColumnAliasArray = array()) {
			...
			if ($objExpandAsArrayNode && 
				...
				if (NotaEntregaCkpt::ExpandArray ($objDbRow, $strAliasPrefix, $objExpandAsArrayNode, $objPreviousItemArray, $strColumnAliasArray)) {
					return false; // db row was used but no new object was created
				}
			}

			// Create a new instance of the NotaEntregaCkpt object
			$objToReturn = new NotaEntregaCkpt();
			...
			if (!$strAliasPrefix)
				$strAliasPrefix = 'nota_entrega_ckpt__';
			...
			if (!is_null($objDbRow->GetColumn($strAliasName))) {
				...
				$objToReturn->objContainer = NotaEntrega::InstantiateDbRow($objDbRow, $strAliasPrefix . 'container_id__', $objExpansionNode, null, $strColumnAliasArray);
			}
		...
		/**
		 * Instantiate an array of NotaEntregaCkpts from a Database Result
		...
		public static function InstantiateDbResult(QDatabaseResultBase $objDbResult, $objExpandAsArrayNode = null, $strColumnAliasArray = null) {
			...
			if ($objExpandAsArrayNode) {
				...
				$objPrevItemArray = array();
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = NotaEntregaCkpt::InstantiateDbRow($objDbRow, null, $objExpandAsArrayNode, $objPrevItemArray, $strColumnAliasArray);
					if ($objItem) {
						$objToReturn[] = $objItem;
						$objPrevItemArray[$objItem->intId][] = $objItem;
				...
				while ($objDbRow = $objDbResult->GetNextRow())
					$objToReturn[] = NotaEntregaCkpt::InstantiateDbRow($objDbRow, null, null, null, $strColumnAliasArray);
			}
		...
		/**
		 * Instantiate a single NotaEntregaCkpt object from a query cursor (e.g. a DB ResultSet).
		 * Cursor is automatically moved to the "next row" of the result set.
		 * Will return NULL if no cursor or if the cursor has no more rows in the resultset.
		 * @param QDatabaseResultBase $objDbResult cursor resource
		 * @return NotaEntregaCkpt next row resulting from the query
		 */
		public static function InstantiateCursor(QDatabaseResultBase $objDbResult) {
			...
			return NotaEntregaCkpt::InstantiateDbRow($objDbRow, null, null, null, $strColumnAliasArray);
		}
		...
		/**
		 * Load a single NotaEntregaCkpt object,
		 * by Id Index(es)
		 * @param integer $intId
		 * @param QQClause[] $objOptionalClauses additional optional QQClause objects for this query
		 * @return NotaEntregaCkpt
		*/
		public static function LoadById($intId, $objOptionalClauses = null) {
			return NotaEntregaCkpt::QuerySingle(
				QQ::AndCondition(
					QQ::Equal(QQN::NotaEntregaCkpt()->Id, $intId)
				),
				$objOptionalClauses
		...
		/**
		 * Load an array of NotaEntregaCkpt objects,
		...
		*/
		public static function LoadArrayByContainerId($intContainerId, $objOptionalClauses = null) {
			// Call NotaEntregaCkpt::QueryArray to perform the LoadArrayByContainerId query
			try {
				return NotaEntregaCkpt::QueryArray(
					QQ::Equal(QQN::NotaEntregaCkpt()->ContainerId, $intContainerId),
					$objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
		...
		/**
		 * Count NotaEntregaCkpts
		 * by ContainerId Index(es)
		 * @param integer $intContainerId
		 * @return int
		*/
		public static function CountByContainerId($intContainerId) {
			// Call NotaEntregaCkpt::QueryCount to perform the CountByContainerId query
			return NotaEntregaCkpt::QueryCount(
				QQ::Equal(QQN::NotaEntregaCkpt()->ContainerId, $intContainerId)
			);
		}
		...
		*/
		public static function LoadArrayByCheckpointId($intCheckpointId, $objOptionalClauses = null) {
			// Call NotaEntregaCkpt::QueryArray to perform the LoadArrayByCheckpointId query
			try {
				return NotaEntregaCkpt::QueryArray(
					QQ::Equal(QQN::NotaEntregaCkpt()->CheckpointId, $intCheckpointId),
					$objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
		...
		*/
		public static function CountByCheckpointId($intCheckpointId) {
			// Call NotaEntregaCkpt::QueryCount to perform the CountByCheckpointId query
			return NotaEntregaCkpt::QueryCount(
				QQ::Equal(QQN::NotaEntregaCkpt()->CheckpointId, $intCheckpointId)
			);
		}
		...
		*/
		public static function LoadArrayBySucursalId($intSucursalId, $objOptionalClauses = null) {
			// Call NotaEntregaCkpt::QueryArray to perform the LoadArrayBySucursalId query
			try {
				return NotaEntregaCkpt::QueryArray(
					QQ::Equal(QQN::NotaEntregaCkpt()->SucursalId, $intSucursalId),
					$objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
		...
		*/
		public static function CountBySucursalId($intSucursalId) {
			// Call NotaEntregaCkpt::QueryCount to perform the CountBySucursalId query
			return NotaEntregaCkpt::QueryCount(
				QQ::Equal(QQN::NotaEntregaCkpt()->SucursalId, $intSucursalId)
			);
		}
		...
		*/
		public static function LoadArrayByUsuarioId($intUsuarioId, $objOptionalClauses = null) {
			// Call NotaEntregaCkpt::QueryArray to perform the LoadArrayByUsuarioId query
			try {
				return NotaEntregaCkpt::QueryArray(
					QQ::Equal(QQN::NotaEntregaCkpt()->UsuarioId, $intUsuarioId),
					$objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
		...
		*/
		public static function CountByUsuarioId($intUsuarioId) {
			// Call NotaEntregaCkpt::QueryCount to perform the CountByUsuarioId query
			return NotaEntregaCkpt::QueryCount(
				QQ::Equal(QQN::NotaEntregaCkpt()->UsuarioId, $intUsuarioId)
			);
		}
		...
		public function Save($blnForceInsert = false, $blnForceUpdate = false) {
			...
			try {
				if ((!$this->__blnRestored) || ($blnForceInsert)) {
					...
					$mixToReturn = $this->intId = $objDatabase->InsertId('nota_entrega_ckpt', 'id');
				} else {
		...
		/**
		 * Delete this NotaEntregaCkpt
		 * @return void
		 */
		public function Delete() {
			if ((is_null($this->intId)))
				throw new QUndefinedPrimaryKeyException('Cannot delete this NotaEntregaCkpt with an unset primary key.');
        ...
        /**
 	     * Delete this NotaEntregaCkpt ONLY from the cache
 		 * @return void
		 */
		public function DeleteCache() {
			if (QApplication::$objCacheProvider && QApplication::$Database[1]->Caching) {
				$strCacheKey = QApplication::$objCacheProvider->CreateKey(QApplication::$Database[1]->Database, 'NotaEntregaCkpt', $this->intId);
				QApplication::$objCacheProvider->Delete($strCacheKey);
			}
		}

		/**
		 * Delete all NotaEntregaCkpts
		 * @return void
		 */
		...
		/**
		 * Truncate nota_entrega_ckpt table
		 * @return void
		 */
		...
		/**
		 * Reload this NotaEntregaCkpt from the database.
		 * @return void
		 */
		public function Reload() {
			// Make sure we are actually Restored from the database
			if (!$this->__blnRestored)
				throw new QCallerException('Cannot call Reload() on a new, unsaved NotaEntregaCkpt object.');

			$this->DeleteCache();

			// Reload the Object
			$objReloaded = NotaEntregaCkpt::Load($this->intId);
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'Container':
					/**
					 * Gets the value for the NotaEntrega object referenced by intContainerId (Not Null)
					 * @return NotaEntrega
					 */
					try {
						if ((!$this->objContainer) && (!is_null($this->intContainerId)))
							$this->objContainer = NotaEntrega::Load($this->intContainerId);
		...
		public function __set($strName, $mixValue) {
			switch ($strName) {
				...
				case 'Container':
					/**
					 * Sets the value for the NotaEntrega object referenced by intContainerId (Not Null)
					...
					if (is_null($mixValue)) {
						...
						if (is_null($mixValue->Id))
							throw new QCallerException('Unable to set an unsaved Container for this NotaEntregaCkpt');
				...
				case 'Checkpoint':
					...
					if (is_null($mixValue)) {
						...
						if (is_null($mixValue->Id))
							throw new QCallerException('Unable to set an unsaved Checkpoint for this NotaEntregaCkpt');
				...
				case 'Sucursal':
					...
					if (is_null($mixValue)) {
						...
						if (is_null($mixValue->Id))
							throw new QCallerException('Unable to set an unsaved Sucursal for this NotaEntregaCkpt');
				...
				case 'Usuario':
					...
					if (is_null($mixValue)) {
						...
						if (is_null($mixValue->CodiUsua))
							throw new QCallerException('Unable to set an unsaved Usuario for this NotaEntregaCkpt');
		...
		public static function GetDatabaseName() {
			return QApplication::$Database[NotaEntregaCkpt::GetDatabaseIndex()]->Database;
		}
		...
		public static function GetSoapComplexTypeXml() {
			$strToReturn = '<complexType name="NotaEntregaCkpt"><sequence>';
			$strToReturn .= '<element name="Id" type="xsd:int"/>';
			$strToReturn .= '<element name="Container" type="xsd1:NotaEntrega"/>';
		...
		public static function AlterSoapComplexTypeArray(&$strComplexTypeArray) {
			if (!array_key_exists('NotaEntregaCkpt', $strComplexTypeArray)) {
				$strComplexTypeArray['NotaEntregaCkpt'] = NotaEntregaCkpt::GetSoapComplexTypeXml();
		...
		public static function GetArrayFromSoapArray($objSoapArray) {
			...
			foreach ($objSoapArray as $objSoapObject)
				array_push($objArrayToReturn, NotaEntregaCkpt::GetObjectFromSoapObject($objSoapObject));
		...
		public static function GetObjectFromSoapObject($objSoapObject) {
			...
			if ((property_exists($objSoapObject, 'Container')) &&
				...
				$objToReturn->Container = NotaEntrega::GetObjectFromSoapObject($objSoapObject->Container);
		...
		public static function GetSoapArrayFromArray($objArray) {
			...
			foreach ($objArray as $objObject)
				array_push($objArrayToReturn, NotaEntregaCkpt::GetSoapObjectFromObject($objObject, true));
		...
		public static function GetSoapObjectFromObject($objObject, $blnBindRelatedObjects) {
			if ($objObject->objContainer)
				$objObject->objContainer = NotaEntrega::GetSoapObjectFromObject($objObject->objContainer, false);
     ...
     * @property-read QQNodeNotaEntrega $Container
     ...
     **/
	class QQNodeNotaEntregaCkpt extends QQNode {
		protected $strTableName = 'nota_entrega_ckpt';
		protected $strPrimaryKey = 'id';
		protected $strClassName = 'NotaEntregaCkpt';
		public function __get($strName) {
			switch ($strName) {
				case 'Id':
				...
				case 'Container':
					return new QQNodeNotaEntrega('container_id', 'Container', 'Integer', $this);
				case 'CheckpointId':
     ...
     * @property-read QQNode $_PrimaryKeyNode
     **/
	class QQReverseReferenceNodeNotaEntregaCkpt extends QQReverseReferenceNode {
		protected $strTableName = 'nota_entrega_ckpt';
		protected $strPrimaryKey = 'id';
		protected $strClassName = 'NotaEntregaCkpt';
		public function __get($strName) {
