<?php
	...
	class NotificacionGen extends QBaseClass implements IteratorAggregate {
		...
		/**
		 * Protected member variable that maps to the database PK Identity column notificacion.id
		 * @var integer intId
		 */
		protected $intId;
		...
		public function Initialize()
		{
			...
			$this->intGuiaId = Notificacion::GuiaIdDefault;
			$this->intCheckpointId = Notificacion::CheckpointIdDefault;
		...
		public static function LoadArrayByGuiaId($intGuiaId, $objOptionalClauses = null) {
			...
			try {
				return Notificacion::QueryArray(
					QQ::Equal(QQN::Notificacion()->GuiaId, $intGuiaId),
					$objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
		...
		*/
		public static function LoadArrayByCheckpointId($intCheckpointId, $objOptionalClauses = null) {
			...
			try {
				return Notificacion::QueryArray(
					QQ::Equal(QQN::Notificacion()->CheckpointId, $intCheckpointId),
					$objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
		...
		public function Save($blnForceInsert = false, $blnForceUpdate = false) {
			...
			try {
				if ((!$this->__blnRestored) || ($blnForceInsert)) {
					...
					$mixToReturn = $this->intId = $objDatabase->InsertId('notificacion', 'id');
				} else {
