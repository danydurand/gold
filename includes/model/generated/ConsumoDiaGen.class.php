<?php
	 ...
	 * @property double $CantidadKilos the value for fltCantidadKilos 
	 * @property double $TotalFacturado the value for fltTotalFacturado 
	...
	class ConsumoDiaGen extends QBaseClass implements IteratorAggregate {
		...
		/**
		 * Protected member variable that maps to the database column consumo_dia.cantidad_kilos
		 * @var double fltCantidadKilos
		 */
		protected $fltCantidadKilos;
		const CantidadKilosDefault = 0;


		/**
		 * Protected member variable that maps to the database column consumo_dia.total_facturado
		 * @var double fltTotalFacturado
		 */
		protected $fltTotalFacturado;
		const TotalFacturadoDefault = 0;
		...
		public function Initialize()
		{
			...
			$this->fltCantidadKilos = ConsumoDia::CantidadKilosDefault;
			$this->fltTotalFacturado = ConsumoDia::TotalFacturadoDefault;
		...
		public static function GetSelectFields(QQueryBuilder $objBuilder, $strPrefix = null, QQSelect $objSelect = null) {
            ...
            if ($objSelect) {
			    ...
			    $objBuilder->AddSelectItem($strTableName, 'cantidad_kilos', $strAliasPrefix . 'cantidad_kilos');
			    $objBuilder->AddSelectItem($strTableName, 'total_facturado', $strAliasPrefix . 'total_facturado');
		...
		*/
		public static function InstantiateDbRow($objDbRow, $strAliasPrefix = null, $objExpandAsArrayNode = null, $objPreviousItemArray = null, $strColumnAliasArray = array()) {
			...
			$strAlias = $strAliasPrefix . 'cantidad_kilos';
			$strAliasName = !empty($strColumnAliasArray[$strAlias]) ? $strColumnAliasArray[$strAlias] : $strAlias;
			$objToReturn->fltCantidadKilos = $objDbRow->GetColumn($strAliasName, 'Float');
			$strAlias = $strAliasPrefix . 'total_facturado';
			$strAliasName = !empty($strColumnAliasArray[$strAlias]) ? $strColumnAliasArray[$strAlias] : $strAlias;
			$objToReturn->fltTotalFacturado = $objDbRow->GetColumn($strAliasName, 'Float');
		...
		*/
				...
				if ((!$this->__blnRestored) || ($blnForceInsert)) {
					...
					$objDatabase->NonQuery('
						INSERT INTO `consumo_dia` (
							...
							' . $objDatabase->SqlVariable($this->fltCantidadKilos) . ',
							' . $objDatabase->SqlVariable($this->fltTotalFacturado) . ',
					...
					$objDatabase->NonQuery('
						...
						SET
							...
							`cantidad_kilos` = ' . $objDatabase->SqlVariable($this->fltCantidadKilos) . ',
							`total_facturado` = ' . $objDatabase->SqlVariable($this->fltTotalFacturado) . ',
		...
		public function Reload() {
			...
			$this->fltCantidadKilos = $objReloaded->fltCantidadKilos;
			$this->fltTotalFacturado = $objReloaded->fltTotalFacturado;
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'CantidadKilos':
					/**
					 * Gets the value for fltCantidadKilos 
					 * @return double
					 */
					return $this->fltCantidadKilos;

				case 'TotalFacturado':
					/**
					 * Gets the value for fltTotalFacturado 
					 * @return double
					 */
					return $this->fltTotalFacturado;
		...
		public function __set($strName, $mixValue) {
			switch ($strName) {
				...
				case 'CantidadKilos':
					/**
					 * Sets the value for fltCantidadKilos 
					 * @param double $mixValue
					 * @return double
					 */
					try {
						return ($this->fltCantidadKilos = QType::Cast($mixValue, QType::Float));
					} catch (QCallerException $objExc) {
						$objExc->IncrementOffset();
						throw $objExc;
					}
				...
				case 'TotalFacturado':
					/**
					 * Sets the value for fltTotalFacturado 
					 * @param double $mixValue
					 * @return double
					 */
					try {
						return ($this->fltTotalFacturado = QType::Cast($mixValue, QType::Float));
					} catch (QCallerException $objExc) {
						$objExc->IncrementOffset();
						throw $objExc;
					}
		...
		public static function GetSoapComplexTypeXml() {
			...
			$strToReturn .= '<element name="CantidadKilos" type="xsd:float"/>';
			$strToReturn .= '<element name="TotalFacturado" type="xsd:float"/>';
		...
		public static function GetObjectFromSoapObject($objSoapObject) {
			...
			if (property_exists($objSoapObject, 'CantidadKilos'))
				$objToReturn->fltCantidadKilos = $objSoapObject->CantidadKilos;
			if (property_exists($objSoapObject, 'TotalFacturado'))
				$objToReturn->fltTotalFacturado = $objSoapObject->TotalFacturado;
		...
		public function getIterator() {
			...
			$iArray['CantidadKilos'] = $this->fltCantidadKilos;
			$iArray['TotalFacturado'] = $this->fltTotalFacturado;
     ...
     * @property-read QQNode $CantidadKilos
     * @property-read QQNode $TotalFacturado
	...
	class QQNodeConsumoDia extends QQNode {
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'CantidadKilos':
					return new QQNode('cantidad_kilos', 'CantidadKilos', 'Float', $this);
				case 'TotalFacturado':
					return new QQNode('total_facturado', 'TotalFacturado', 'Float', $this);
				case 'CreatedAt':
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'CantidadGuias':
					return new QQNode('cantidad_guias', 'CantidadGuias', 'integer', $this);
				case 'CantidadKilos':
					return new QQNode('cantidad_kilos', 'CantidadKilos', 'double', $this);
				case 'TotalFacturado':
					return new QQNode('total_facturado', 'TotalFacturado', 'double', $this);
				case 'CreatedAt':
