...
class Guias extends GuiasGen
{
    ...
    public function createPiecesBackup() {
        ...
        // t('==========================');
        $objParaExis = Parametros::BP('PiecBack', $this->Id, 'TODO');
        $blnCreaBack = false;
        if ($objParaExis instanceof Parametros) {
            // t('There is a Parameter. Parameter pieces count: '. $objParaExis->Valor1);
            if ($objParaExis->Valor1 != $this->Piezas) {
                // t('The Parameter must be updated, because there are differents quantities of pieces');
                $objParaExis->Delete();
                $blnCreaBack = true;
            }
        } else {
            // t('There is no Backup Parameter');
            $blnCreaBack = true;
        }
        if ($blnCreaBack) {
            // t('Creating the pieces backup parameter');
            $strPiecJson = '';
            $intPiecQnty = 0;
            foreach ($this->GetGuiaPiezasAsGuiaArray() as $objPiecAwbx) {
    ...
    public function getPiecesBackup()  {
        // t('Rutine: createPiecesBackup');
        // t('==========================');
        $objParaExis = Parametros::BP('PiecBack', $this->Id, 'TODO');
        if ($objParaExis instanceof Parametros) {
            // t('There are: '.$objParaExis->Valor1.' pieces in the backup record');
            $arrPiecBack = explode('*',$objParaExis->Texto1);
            // t('The array contains: '.count($arrPiecBack).' elements');
            foreach ($arrPiecBack as $strPiecJson) {
                // t('Json: '.$strPiecJson);
                try {
                    $objNewxPiec = new GuiaPiezas();
                    $objPiecRcry = json_decode($strPiecJson);
                    $objNewxPiec->GuiaId        = $objPiecRcry->GuiaId;
                    $objNewxPiec->NotaEntregaId = $objPiecRcry->NotaEntregaId;
                    $objNewxPiec->IdPieza       = $objPiecRcry->IdPieza;
                    $objNewxPiec->Kilos         = $objPiecRcry->Kilos;
                    $objNewxPiec->EmpaqueId     = $objPiecRcry->EmpaqueId;
                    $objNewxPiec->Libras        = $objPiecRcry->Libras;
                    $objNewxPiec->Largo         = $objPiecRcry->Largo;
                    $objNewxPiec->Alto          = $objPiecRcry->Alto;
                    $objNewxPiec->Ancho         = $objPiecRcry->Ancho;
                    $objNewxPiec->Volumen       = $objPiecRcry->Volumen;
                    $objNewxPiec->Descripcion   = $objPiecRcry->Descripcion;
                    $objNewxPiec->PiesCub       = $objPiecRcry->PiesCub;
                    $objNewxPiec->MetrosCub     = $objPiecRcry->MetrosCub;
                    $objNewxPiec->HojaEntrega   = $objPiecRcry->HojaEntrega;
                    $objNewxPiec->Ubicacion     = $objPiecRcry->Ubicacion;
                    $objNewxPiec->CreatedAt     = $objPiecRcry->CreatedAt;
                    $objNewxPiec->UpdatedAt     = $objPiecRcry->UpdatedAt;
                    $objNewxPiec->Save();
                    // t('Recovered piece');
                    if (!is_null($objPiecRcry->LastCkptId)) {
                        $objCkptPiez = new PiezaCheckpoints();
                        $objCkptPiez->PiezaId      = $objNewxPiec->Id;
                        $objCkptPiez->CheckpointId = Checkpoints::LoadByCodigo($objPiecRcry->LastCkptCode)->Id;
                        $objCkptPiez->SucursalId   = $objPiecRcry->LastCkptSucursalId;
                        $objCkptPiez->Fecha        = new QDateTime($objPiecRcry->LastCkptDate->date);
                        $objCkptPiez->Hora         = $objPiecRcry->LastCkptHour;
                        $objCkptPiez->Comentario   = $objPiecRcry->LastCkptComment;
                        $objCkptPiez->Save();
                        // t('Last Ckpt Recovered');
                    }
