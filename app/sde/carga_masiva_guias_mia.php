<?php
//--------------------------------------------------------------------------------------------------------------------
// Programa       : carga_masiva_guias_mia.php
...
class CargaMasivaGuiasMia extends FormularioBaseKaizen {
    protected $objTarifa;
    ...
    protected function Form_Create() {
        ...
        if ($this->blnEditMode) {
            $this->lblTituForm->Text = 'Consulta de Manifiesto Miami';
        } else {
            $this->lblTituForm->Text = 'Carga de Manifiesto Miami';
        }
    ...
    protected function btnMasxAcci_Create() {
        ...
        if ($this->blnEditMode) {
            $_SESSION['PagiBack'] = 'carga_masiva_guias_mia.php/'.$this->objNotaEntr->Id;
            $blnCambStat = false;
    ...
    protected function createdtgGuiaManiColumns() {
        ...
        $colNumeTula = new QDataGridColumn($this);
        $colNumeTula->Name = 'Tula';
        $colNumeTula->Html = '<?= $_ITEM->NroTula; ?>';
        $this->dtgGuiaMani->AddColumn($colNumeTula);
    ...
    protected function btnSave_Click() {
        ...
        QApplication::Redirect(__SIST__.'/carga_masiva_guias_mia.php/'.$this->objNotaEntr->Id);
    }
    ...
    protected function verificarDatosMasivos($arrCampClie,$intNumeLine) {
        ...
        $strNumeTula = strtoupper(trim($arrCampClie[7]));
        $arrContVali['NumeTula'] = $strNumeTula;
        if ($blnTodoOkey) {
            if (strlen($strNumeTula) < 0) {
                $strTextErro = "Linea $intNumeLine | Col 8 | Nro de Tula es requerido.";
                $blnTodoOkey = false;
            }
        }
        ...
        try {
            ...
            foreach ($xls->rows() as $arrCampClie) {
                if ( (count($arrCampClie) > 0) && ($intNumeLine > 1) ) {
                    ...
                    if ($intCantCamp != 8) {
                        $strMensErro = "La linea $intNumeLine no tiene los 8 campos requeridos";
                        t($strMensErro);
                        $arrParaErro['ProcIdxx'] = $this->objProcEjec->Id;
                        $arrParaErro['NumeRefe'] = $strNombArch;
                        $arrParaErro['MensErro'] = $strMensErro;
                    ...
                    $strNumeTula = $arrContVali['NumeTula'];
                    //----------------------------------------------------------------
                    ...
                    try {
                        ...
                        $objGuiaMasi->CodigoContrato      = $strNumeTula;
    ...
    protected function crearGuiaMasiva(GuiaCacesa $objGuiaMasi, $objCkptProc) {
        ...
        try {
            ...
            $objGuia->NroTula               = $objGuiaMasi->CodigoContrato;
...
CargaMasivaGuiasMia::Run('CargaMasivaGuiasMia');
