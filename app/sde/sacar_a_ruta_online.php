...
class SacarARutaOnline extends FormularioBaseKaizen {
    ...
    protected function txtPiezScan_Create() {
        $this->txtPiezScan = new QTextBox($this);
        $this->txtPiezScan->Width = 185;
        $this->txtPiezScan->AddAction(new QEnterKeyEvent(), new QAjaxAction('txtPiezScan_EnterKey'));
    }

    protected function txtResuScan_Create() {
        $this->txtResuScan = new QTextBox($this);
        $this->txtResuScan->Width = 185;
    ...
    protected function txtListNume_Create() {
        ...
        $this->txtListNume->Rows = 8;
        //$this->txtListNume->Enabled = false;
        //$this->txtListNume->ForeColor = 'blue';
    }
    ...
    protected function txtPiezScan_EnterKey() {
        $this->mensaje();
        $this->txtResuScan->Text = '';
        $this->txtResuScan->ForeColor = null;
        $intOperIdxx = $this->lstOperAbie->SelectedValue;
        if (is_null($intOperIdxx)) {
            $this->danger('Debe seleccionar una Ruta');
            return;
        }
        $objOperRuta = SdeOperacion::Load($intOperIdxx);
        $arrDestinos = $objOperRuta->GetDestinosIds();

        $strPiezScan = transformar(trim($this->txtPiezScan->Text));
        if (strlen($strPiezScan) > 0) {
            $objGuiaPiez = GuiaPiezas::LoadByIdPieza($strPiezScan);
            if (!$objGuiaPiez) {
                $this->txtResuScan->ForeColor = 'red';
                $this->txtResuScan->Text = 'LA PIEZA NO EXISTE';
                return;
            }
            if (!in_array($objGuiaPiez->Guia->DestinoId,$arrDestinos)) {
                $this->txtResuScan->ForeColor = 'red';
                $this->txtResuScan->Text = 'DESTINO NO COINCIDE';
                return;
            }
            $this->txtListNume->Text .= $strPiezScan.chr(13);
            $this->txtResuScan->ForeColor = 'green';
            $this->txtResuScan->Text = 'OKEY';
            $this->txtPiezScan->Text = '';
        }
        $this->txtPiezScan->SetFocus();
    }
...
SacarARutaOnline::Run('SacarARutaOnline');
?>